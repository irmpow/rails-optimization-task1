...
def work(file_name = DATA_FILE)
  ...
  collect_stats_from_users(report, users_objects) do |user|
    { 'sessionsCount' => user.sessions.nil? ? 0 : user.sessions.count }
  end
  ...
  collect_stats_from_users(report, users_objects) do |user|
    { 'totalTime' => user.sessions.nil? ? 0 : user.sessions.map {|s| s['time']}.map {|t| t.to_i}.sum.to_s + ' min.' }
  end
  ...
  collect_stats_from_users(report, users_objects) do |user|
    { 'longestSession' => user.sessions.nil? ? 0 : user.sessions.map {|s| s['time']}.map {|t| t.to_i}.max.to_s + ' min.' }
  end
  ...
  collect_stats_from_users(report, users_objects) do |user|
    { 'browsers' => user.sessions.nil? ? "" : user.sessions.map {|s| s['browser']}.map {|b| b.upcase}.sort.join(', ') }
  end
  ...
  # Хоть раз использовал IE?
  collect_stats_from_users(report, users_objects) do |user|
    { 'usedIE' => user.sessions.nil? ? false : user.sessions.map{|s| s['browser']}.any? { |b| b.upcase =~ /INTERNET EXPLORER/ } }
  end
  ...
  collect_stats_from_users(report, users_objects) do |user|
    { 'alwaysUsedChrome' => user.sessions.nil? ? false : user.sessions.map{|s| s['browser']}.all? { |b| b.upcase =~ /CHROME/ } }
  end
  ...
  collect_stats_from_users(report, users_objects) do |user|
    { 'dates' => user.sessions.nil? ? [] : user.sessions.map{|s| s['date']}.map {|d| Date.parse(d)}.sort.reverse.map { |d| d.iso8601 } }
  end
...
')
  ...
  def execution_not_regressed(start_time, end_time, file_name = DATA_FILE)
    ...
    when 'data16.txt'
      current_test_results = 12
    when 'data32.txt'
      current_test_results = 33
    when 'data64.txt'
      current_test_results = 118
    when 'data128.txt'
      current_test_results = 10
    when 'data_large.txt'
